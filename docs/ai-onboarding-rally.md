...
3. **表示名**（例: カレンダー上の「大原_MS」）をユーザーが言う場合、**`set-context` には `calendarIdentifier`（`CALENDAR_ID`）** を使う。人が見えるタイトル行だけに依存しない。  
...
## 分岐 C: macOS（EventKit / 例: 表示名「大原_MS」）
...
3. ローカル **カレンダー一覧**（**`CALENDAR_ID` = `calendarIdentifier`** 列）:
   ...
   表の **TITLE** が「大原_MS」などユーザーが言う名前に相当する行を探し、同じ行の **CALENDAR_ID** を採用する。  
4. コンテキスト（例）:

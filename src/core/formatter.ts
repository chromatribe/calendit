...
export class Formatter {
  ...
  static fromCsv(csv: string): CalendarEvent[] {
    ...
    return records as CalendarEvent[];
  }
   ...
   * インデントされた行は説明文として扱う
   */
  static fromMarkdown(md: string, strict = false): ParseResult {
  ...
  private static groupByDate(events: CalendarEvent[]) {
  ...
  private static formatTime(iso: string) {
    if (!iso) return "??:??";

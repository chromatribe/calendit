...
export function registerCalCommands(program: Command, deps: CommandDeps) {
  ...
  calCmd
    ...
    .option("--set <name>", "Use a named context")
    .action(async (name: string, options: { set?: string }) => {
      const { service } = await getServiceForContext(deps, options.set);
  ...
  calCmd
    ...
    .option("--set <name>", "Use a named context")
    .action(async (id: string, options: { force: boolean; set?: string }) => {
      if (id === "primary") {
